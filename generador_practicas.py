...
class Entrega:
    ...
    evaluacion_id: Optional[int]  # Agregar este campo
...
class GeneradorPracticasExtendido:
    def __init__(self, config: Dict[str, str]):
        ...
        self.config = config
        self.modelo_ml = GeneradorPracticasML()  # Instancia de GeneradorPracticasML
    ...
    def crear_evaluacion_para_practica(self, practica_id: int, estudiante_id: int) -> Optional[int]:
        ...
        try:
            ...
            return evaluacion_id  # Devolver el ID de la evaluación
        except Error as e:
    ...
    def obtener_semestres(self):
        ...
        try:
            ...
            return self.cursor.fetchall()  # Asegúrate de que esto devuelva una lista de diccionarios o tuplas
        except Error as e:
    ...
    def obtener_estudiantes(self):
        ...
        try:
            query = "SELECT * FROM usuarios WHERE rol = 'estudiante'"  # Asegúrate de que el rol esté definido correctamente
            self.cursor.execute(query)
            return self.cursor.fetchall()  # Asegúrate de que esto devuelva una lista de diccionarios o tuplas
        except Error as e:
    ...
    def obtener_estudiantes_por_practica(self, practica_id):
        ...
        query = """
            SELECT 
                ...
                ev.id AS evaluacion_id  -- Obtener el ID de la evaluación
            FROM 
                evaluaciones ev
            JOIN 
    ...
    def obtener_estudiantes_por_grupo(self, grupo_id):
        ...
        try:
            ...
            """
            self.cursor.execute(query, (grupo_id,))
            return self.cursor.fetchall()  # Devuelve una lista de estudiantes
        except Error as e:
    ...
    def obtener_grupo_actual(self, usuario_id):
        ...
        try:
            ...
            return self.cursor.fetchone()  # Devuelve el grupo actual
        except Error as e:
    ...
    def recopilar_datos_entrenamiento(self) -> Tuple[List[str], List[str], List[str]]:
        ...
        for p in practicas:
            ...
            contenido = f"{titulo} {objetivo}".strip().lower()  # Combina título y objetivo como contenido
    ...
    def registrar_usuario(self, nombre: str, email: str, password: str, rol: str = 'estudiante') -> None:
        ...
        try:
            ...
            password_hash = hashlib.sha256(password.encode()).hexdigest()  # Hashear la contraseña
    ...
    def autenticar_usuario(self, email: str, password: str) -> Optional[Dict]:
        ...
        try:
            ...
            if usuario:
                ...
                # Depuración: Imprimir el hash de la contraseña
                ...
                if password_hash == usuario['password_hash']:
                    # Depuración: Imprimir rol del usuario
                    return {
                        'id': usuario['id'],
            ...
            # Si no se encuentra el usuario o el hash no coincide
    ...
    def actualizar_rol_usuario(self, usuario_id, nuevo_rol):
        ...
        try:
            ...
            return nuevo_rol  # Devuelve el nuevo rol si se actualiza correctamente
        except Exception as e:
            self.connection.rollback()  # Revertir cambios en caso de error
            print(f"Error al actualizar rol: {str(e)}")  # Mensaje de depuración
            return None
    ...
    def obtener_contenido_generado(self, practica_id: int) -> Optional[dict]:
        ...
        try:
            ...
            return None  # Asegúrate de que esto esté correctamente escrito
    ...
    def obtener_evaluaciones(self):
       ...
       try:
           ...
           print("Evaluaciones obtenidas:", evaluaciones)  # Depuración
           return evaluaciones
       except Exception as e:
    ...
    def evaluar_entrega(self, evaluacion_id: int, contenido: str, titulo: str, objetivo: str):
        ...
        try:
            ...
            # Actualizar la evaluación con la calificación y comentarios generados
    ...
    def actualizar_evaluacion(self, evaluacion_id, comentarios, calificacion):
        ...
        """
        try:
            # Asegurar que calificacion sea float
            if calificacion is not None:
                calificacion = float(calificacion)
        
            # Convertir comentarios a string si es un diccionario
            if isinstance(comentarios, dict):
                comentarios = json.dumps(comentarios)
            elif comentarios is not None:
            ...
            # Actualizar el estado de la entrega asociada
    ...
    def obtener_grupos_por_profesor(self, profesor_id):
        ...
        try:
            ...
            resultados = self.cursor.fetchall()  # Devuelve una lista de grupos
    ...
    def obtener_entrega_por_id(self, entrega_id: int, es_evaluacion: bool = False) -> Optional[Dict]:
        try:
            ...
            print(f"Buscando entrega con ID: {entrega_id}")  # Depuración
            query = """
            SELECT e.id AS entrega_id, 
                   e.practica_id, 
                   e.estudiante_id, 
            ...
            print(f"Resultado de la consulta de entrega: {entrega}")  # Depuración
            return entrega
        except mysql.connector.Error as e:
    ...
    def obtener_entrega_desde_evaluacion(self, evaluacion_id: int) -> Optional[int]:
        """Obtiene el ID de la entrega asociada a una evaluación."""
        try:
            print(f"Buscando entrega asociada a la evaluación con ID: {evaluacion_id}")  # Depuración
            query = """
            ...
            self.cursor.execute(query, (evaluacion_id,))
            result = self.cursor.fetchone()
            print(f"Resultado de obtener_entrega_desde_evaluacion: {result}")  # Depuración
            return result['entrega_id'] if result else None
        except mysql.connector.Error as e:
    ...
    def crear_practica(self, practica: Practica):
        try:
            ...
            print(f"Práctica creada con ID: {practica_id}")  # Depuración
    ...
    def crear_evaluaciones_para_practica(self, practica_id: int, autor_id: int, uso_ia: bool = False) -> None:
        ...
        try:
            ...
            for estudiante in estudiantes:
                ...
                print(f"Evaluación creada para estudiante ID: {estudiante_id}")  # Depuración
    ...
    def actualizar_calificacion_entrega(self, evaluacion_id: int, estudiante_id: int) -> bool:
        ...
        try:
            ...
            if resultado_calificacion:
                ...
                # Actualizar la calificación en la tabla de entregas usando evaluacion_id
                query_actualizar = """
                    UPDATE entregas
                    SET calificacion = %s
    ...
    def obtener_entrega_por_evaluacion(self, evaluacion_id):
        ...
        try:
            ...
            # Primero intentamos obtener el ID de la entrega
            entrega_id = self.obtener_entrega_desde_evaluacion(evaluacion_id)

            if entrega_id:
                # Si encontramos el ID, obtenemos la entrega completa
                query = """
            ...
            # Si no encontramos la entrega por ID, intentamos por practica_id y estudiante_id
            # Primero obtenemos la evaluación para conocer practica_id y estudiante_id
            query_eval = """
            ...
            # Ahora buscamos la entrega correspondiente
            query = """
            SELECT id, practica_id, estudiante_id, fecha_entrega, estado, 
                   archivos_url, contenido, evaluacion_id
            FROM entregas
            ...
            if entrega:
                ...
                # Si la entrega no tiene asociada la evaluación, actualizarla
    ...
    def registrar_entrega(self, entrega: Dict[str, Any]) -> Optional[int]:
        ...
        """
        try:
            # Verificar si ya existe una entrega para esta práctica y estudiante
            query_check = """
    ...
    def obtener_evaluaciones_calificadas(self, estudiante_id):
        ...
        try:
            ...
            # Agregar subdiccionario 'practica' a cada evaluación
            for evaluacion in evaluaciones:
                evaluacion["practica"] = {
                    "id": evaluacion["practica_id"],
                    "titulo": evaluacion["practica_titulo"]
                }
